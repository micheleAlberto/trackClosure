...
class MergeOutcome(Exception):
    ...
    def is_correct(self):
        correct_operation = (self.has_correct_results()
                            if self.has_correct_operands()
                            else True)
        return correct_operation

    def is_known_correct(self):
        known_correct_operation = (
            self.has_known_correct_results()
            if self.has_known_correct_operands()
            else True)
        return known_correct_operation

    def is_recover(self):
        return self.has_correct_results() and not self.has_correct_operands()

    def is_known_recover(self):
        return self.has_known_correct_results() and not self.has_correct_operands()
